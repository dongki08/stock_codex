...
>
> **2026-05-22 대규모 개정**: P0/P1/P2 결함 19개 중 16개 적용 완료. 자세한 결함/수정 이력은 별도 문서 `BACKEND_DEFECTS_AND_FIXES.md` 참고. 본 문서는 현재 코드 기준으로 갱신됨.
> 주요 추가/변경:
...
> - ExitConfirm 룰 우선 → Codex 후순위 (Codex 비용 절감)
...
9. [ExitConfirm — 손절 위험구간 보조 판단](#9-exitconfirm--손절-위험구간-보조-판단)
10. [스케줄러 6종 — 자동 동작 시각표](#10-스케줄러-6종--자동-동작-시각표)
...
[08:30]     KRX 프리오픈                      ← 한국 심볼 동기화 → 일봉 → 추천 생성 → 텔레그램 발송
[09:00~15:30] 평일 폴링 ExitMonitor          ← OPEN 추천 감시: target/stop/expiry 자동 평가 + 룰 우선 ExitConfirm
[21:40]     US 뉴스/공시/펀더멘털 수집
[22:00/23:00] US 프리오픈 (DST에 따라)        ← 미국 심볼 동기화 → 일봉 → 추천 → 텔레그램
...
| **가격 데이터 수집** | `MarketDataSyncService`, `KisApiClient`, `StooqQuoteClient` | KIS(한국) / Stooq(미국)에서 일봉/장중 가격을 받아 `price_daily`, `price_intraday`에 저장 |
...
| **장중 감시 + 자동 청산** | `ExitMonitorJob`, `ExitConfirmService` | 폴링: target/stop/expiry 도달 시 `evaluation` 생성 + 상태 전환. 위험구간 진입 시 룰 우선 → 모호하면 Codex |
...
└──────────────────────────────────────────────────────────────────────┘
            ...
            ▼  (이후 장중에 폴링)
┌──────────────────────────────────────────────────────────────────────┐
│  ExitMonitorJob (장중 09~15시, exit.polling.intervalMinutes, MON-FRI) │
│  ▸ KIS 현재가 조회 → price_intraday 저장                              │
│  ▸ 목표가 도달 → 🎯 priceBucket dedupe 알림 + 자동 평가/CLOSED        │
│  ▸ 손절가 이탈 → 🛑 알림 + 자동 평가/CLOSED                           │
│  ▸ expectedExitAt 경과 → 자동 평가/EXPIRED                            │
│  ▸ 손절 위험 구간 → ExitConfirmService                                │
│       1차 룰: current ≤ stop → CUT (Codex 미호출)                     │
│       모호하면 Codex 호출 → HOLD/CUT/TIGHTEN                          │
│       exit_confirm_log 저장 + (CUT/TIGHTEN이면) ⚠️ 알림                │
└──────────────────────────────────────────────────────────────────────┘
            │
            ▼  (ExitMonitor가 자동 생성)
┌──────────────────────────────────────────────────────────────────────┐
...
- **Flyway** (스키마 마이그레이션, `db/migration/V1..V6`)
- **MSSQL** (`mssql-jdbc`, SQL Server Dialect)
- **springdoc-openapi 2.6.0** (Swagger UI: `/swagger-ui.html`)
- **Lombok**
- **JUnit 5** (현재 테스트 2개: `ResultDtoTest`, `GlobalExceptionHandlerTest`)
...
├── config/                            // Spring 설정, @ConfigurationProperties
│   ├── AppConfig.java                 //   @EnableScheduling, @ConfigurationPropertiesScan
│   ├── SecurityConfig.java            //   BasicAuth + CORS
│   ├── KisProperties / TelegramProperties / DartProperties / SecProperties / CodexCliProperties
│   ├── AdminProperties / AppSecurityProperties
├── api/                               // 컨트롤러 + 요청/응답 DTO
...
├── scheduler/                         // @Scheduled 7종 (AutoResearchJob 추가)
│   ├── AutoResearchJob (TUE-SAT 02:20 KST)
│   ├── KrxPreOpenJob · UsPreOpenJob · UsCloseSummaryJob
│   ├── ExitMonitorJob · MarketDataCollectionJob · SchedulerSettingReader
...
- `exit_confirm_log` : ExitConfirm이 돌 때마다 한 줄. `notify_key` UNIQUE 인덱스(WHERE not null)로 같은 이벤트의 중복 알림 차단.
...
## 9. ExitConfirm — 손절 위험구간 보조 판단 (룰 우선 + Codex 후순위)

OPEN 추천이 손절가에 가까워질 때 "그냥 들고 있을지(HOLD), 손절할지(CUT), 손절선을 좁힐지(TIGHTEN)" 를 **먼저 룰로 판단하고, 모호한 케이스만 Codex CLI에게 질의**하는 모듈.

**2026-05-22 변경 (F-1)**: 이전엔 위험구간 진입 = 무조건 Codex 호출이었음. 현재는 명확 룰 케이스 → 즉결, Codex는 모호 케이스만. Codex 비용 50% 이상 절감.

### 9.1 트리거 위치

- 수동: `POST /api/recommendations/{id}/exit-confirm`
- 자동: `ExitMonitorJob`이 매 5분(평일 09~15시) OPEN 추천 전체를 돌면서 손절 위험 구간 진입 시 호출

### 9.2 위험 구간 판정

```java
threshold = stopPrice × (1 + riskBandPercent / 100)
isRiskZone = currentPrice <= threshold
```
`exit.riskBand.percent` 기본 2.0 → "현재가가 손절가의 102% 이내" 면 위험 구간.
...
1. 추천 조회 → 없으면 404
2. 최신 가격 조회
   ▸ price_intraday 1건 우선
   ▸ 없으면 price_daily 최신 1건 (시각은 15:30으로 가정)
   ▸ 둘 다 없으면 action="DATA_REQUIRED" 응답
3. distancePct = (currentPrice - stopPrice) / stopPrice × 100
4. 위험구간 아니면 → action="HOLD", 종료 (Codex 미호출)
5. 위험구간이면 **1차 룰 판단** (decideByRule):
   ▸ current <= stopPrice → 즉시 "CUT", Codex 미호출, usedFallback=false
   ▸ (룰 판단이 명확하면 여기서 종료)
6. 룰이 모호하면 Codex CLI 호출:
     프롬프트: "ACTION: HOLD/CUT/TIGHTEN" 첫 줄 강제
     컨텍스트: recommendationId, market, ticker, term, entry/target/stop, current, distance, confidence, signalsJson
7. Codex 응답 파싱 (parseAction):
     첫 줄에 ^ACTION:\s*(HOLD|CUT|TIGHTEN)$ 정규식 매칭 → 액션 추출
     미매치 시 안전 HOLD (O-9 해결)
8. Codex 실패 fallback:
     ▸ current <= stopPrice → "CUT"
     ▸ 그 외 → "TIGHTEN"
9. usedFallback, rationale, action, distancePct 등을 응답
```

→ Codex가 진짜 필요한 상황: 손절가 살짝 위에 머무는 모호 구간 + 추세 회복 가능성 판단.

### 9.4 자동 호출 한도 (`ExitMonitorJob.canAutoConfirm`)

스케줄에서만 적용되는 안전장치:
- **스케줄 1회당 한도**: `exit.codex.confirmLimitPerRun` (기본 5건)
- **종목별 일일 한도**: `exit.codex.confirmLimitPerTickerDaily` (기본 3건)
- **추천별 쿨다운**: `exit.codex.confirmCooldownMinutes` (기본 60분, 같은 추천을 60분 안에 재호출 안 함)

→ Codex 호출 폭주를 막는 다층 방어. `ExitConfirmService` 자체에는 한도가 없으므로 수동 API는 호출 가능.

### 9.5 알림

`ExitMonitorJob`은 action이 `CUT` 또는 `TIGHTEN` 일 때만 텔레그램 발송 (`HOLD`는 발송 안 함).
메시지에는 distancePct, fallback 여부, rationale이 포함되며 `notify_key`로 같은 날 동일 추천+액션은 중복 발송 방지.

---

## 10. 스케줄러 7종 — 자동 동작 시각표
...
| `KrxPreOpenJob` | `0 * * * * MON-FRI` | 08:30 | KR 심볼 동기화 → KOSPI 일봉 30개 → 추천 생성 → 텔레그램 |
| `UsPreOpenJob` | `0 * * * * MON-FRI` | 22:00 (DST) / 23:00 (표준) | US 심볼 → NASDAQ 가격 50개 → NASDAQ 일봉 30개 → 추천 → 텔레그램 |
| `UsCloseSummaryJob` | `0 * * * * TUE-SAT` | 05:30 (DST) / 06:30 (표준) | OPEN 미국 추천 전체에 대해 종가 기준 PnL/손절거리 요약 + 하위 5건 → 텔레그램 |
| `ExitMonitorJob` | `0 * 9-15 * * MON-FRI` | 평일 09~15시, `exit.polling.intervalMinutes` 주기 | 현재가 조회 → 자동 평가/청산 → 위험구간 시 룰 우선 ExitConfirm |
...
### 10.5 ExitMonitorJob 세부 로직 (자동 평가/청산 추가)

```
for each recommendation in 추천.status=OPEN:
    if market not in (KOSPI, KOSDAQ): skip
    currentPrice = KisApiClient.fetchCurrentPrice(ticker)
    if 없음: skip (개발모드 또는 외부 실패)
    MarketDataSyncService.saveIntradayPrice(...)

    # 자동 청산 + 평가 생성 (F-12 부분)
    targetReached = target <= current
    stopBreached = stop >= current
    if targetReached: 🎯 priceBucket dedupe 알림
    if stopBreached:  🛑 priceBucket dedupe 알림
    if targetReached or stopBreached:
        closeRecommendationWithEvaluation(...)
            → evaluation 자동 생성 (exitReason=TARGET_HIT/STOP_HIT)
            → recommendation.status = "CLOSED"
        continue
    if today > expectedExitAt:
        closeRecommendationWithEvaluation(...)
            → evaluation (exitReason=TIME_OUT)
            → status = "EXPIRED"
        continue

    # 위험구간 ExitConfirm (룰 우선)
    if current in 손절 위험구간 (current <= stop * 1.02):
        if 스케줄/일일/쿨다운 한도 모두 OK:
            ExitConfirmService.confirm(...)
                ▸ 1차 룰: current <= stop → 즉시 CUT (Codex 미호출)
                ▸ 모호하면 Codex 호출
            exit_confirm_log 저장
            (CUT/TIGHTEN이면) ⚠️ 텔레그램 알림
```

**핵심 변경**: 이전엔 사람이 수동으로 `PUT /api/recommendations/{id}/status`로 상태 전환했지만, 이제 ExitMonitor가 자동으로 평가 + 청산까지 처리. `StatsService.getSummary()`의 통계가 실시간으로 쌓임 → `RecommendationConfidenceService` 학습 가능.
...
| POST | `/api/recommendations/{id}/exit-confirm` | ExitConfirmService.confirm |
...
| GET | `/api/feature/universe-features?market&limit` | UniverseFeatureBuilder 결과 조회 (15개 sub-score 포함 JSON) |
...
| GET/POST | `/api/notifications` | 알림 로그 |
...
# 4) feature 점수 자세히 보기
curl "http://localhost:8083/api/feature/universe-features?market=KOSPI&limit=5"
```
...
# 4) 일봉 동기화 (KR은 KIS 키 필요, US는 Stooq로 가능)
...
5. ExitConfirm 룰 결정 / Codex 결정 비율 90:10 유지 확인 (`exit_confirm_log`)
