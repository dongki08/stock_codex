...
>
> **2026-06-04 재정리 (현 시점)**: 실제 코드 기준으로 패키지 수·테스트 수·Flyway V13·Properties 클래스·API 경로 누락분 보정. 본 문서는 현재 코드 기준으로 갱신됨.
>
> 주요 추가/변경:
...
[08:15]     KRX 뉴스/공시/펀더멘털 수집       ← Google News RSS + Naver 뉴스, DART, SEC
...
| **컨텍스트 데이터 수집** | `MarketDataCollectionService`, `RssNewsClient`, `NaverNewsClient`, `DisclosureClient`, `FredMacroClient`, `SecFundamentalClient`, `MarketSignalScorer` | 뉴스, 공시, 매크로, 펀더멘털을 외부에서 받아 4종 테이블에 저장. 부정문 처리 포함 감성/중요도 점수 부여 |
...
│  StooqQuoteClient · RssNewsClient · NaverNewsClient ·              │
...
stock-advisor.naver-news.client-id: ${NAVER_CLIENT_ID:dev-placeholder}
stock-advisor.naver-news.client-secret: ${NAVER_CLIENT_SECRET:dev-placeholder}
...
│   │   ├── news/RssNewsClient · NaverNewsClient
...
| 테이블 | PK | 용도 |
|---|---|---|
| `news_article` | `article_key (= "NEWS:" + sha256(source+url))` | 멀티소스 뉴스 |
...
- 클래스: `RssNewsClient`, `NaverNewsClient`
- 시장별 출처: KR은 Google News RSS + Naver 뉴스 검색 API, US는 Yahoo Finance RSS + Google News RSS.
- 출처별 기본 5건을 가져와 URL 중복 제거 후 종목당 최대 10건 저장.
- XML 파서는 보안 강화(`disallow-doctype-decl=true`).
- RSS와 Atom 둘 다 지원. `pubDate`/`updated` 파싱 fallback (RFC_1123 → OffsetDateTime) 후 KST로 정규화한다.
- KST 기준 수집 실행일 당일 뉴스만 반환·저장한다. 추천 feature와 Telegram DailyBrief도 KST 당일 뉴스만 읽는다. 한국 종목은 `회사명 종목코드 주식` 검색어를 사용한다.
- Naver 응답은 `originalLink`를 우선 사용하고 제목/요약의 HTML 태그와 entity를 정리한다. `NAVER_CLIENT_ID`, `NAVER_CLIENT_SECRET`이 없으면 Naver만 비활성화된다.
...
runKrxPreOpenCollection (08:15):
    for market in [KOSPI, KOSDAQ]:
        for ticker in 거래대금 상위 N개 (기본 20):
            KST 당일 멀티소스 뉴스 수집
    for market in [KOSPI, KOSDAQ]:
        DART 공시 수집 (limit 20)
...
| GET/POST | `/api/market-data/news` `/news/sync` | 멀티소스 뉴스 |
...
| `NAVER_CLIENT_ID` / `NAVER_CLIENT_SECRET` | dev-placeholder | NaverNewsClient (KR) |
