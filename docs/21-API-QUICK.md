...
| 성과 통계 | `/api/stats` | 종료 추천 성과와 OPEN 추천의 페이퍼트레이딩 미실현 손익을 조회한다. |
...
종료된 추천의 사후 성과와 아직 OPEN 상태인 추천의 페이퍼트레이딩 미실현 손익을 조회한다.
...
| GET | `/api/stats/paper-trading` | OPEN 추천의 최신 일봉 기준 미실현 손익, 비중 반영 손익, 목표/손절 터치 현황 조회 |
...
| pricedCount | Integer | 최신 가격이 있어 손익 계산 가능한 추천 수 |
| avgUnrealizedPnlPct | Decimal | 단순 평균 미실현 손익률 |
| weightedUnrealizedPnlPct | Decimal | `signalsJson.positionWeightPct` 반영 미실현 포트폴리오 손익률 |
| totalWeightPct | Decimal | 가격이 있는 OPEN 추천의 총 비중 |
| targetTouchCount | Integer | 최신 종가가 목표가 이상인 추천 수 |
| stopTouchCount | Integer | 최신 종가가 손절가 이하인 추천 수 |
| positions | Array | 추천별 현재가, 거래일, 비중, 미실현 손익, 목표/손절 이격률 |
...
| POST | `/api/dev/notifications/test?message=hello` | Telegram 테스트 발송 및 `notification_log` 기록 |
...
| sent | Boolean | Telegram 테스트 발송 성공 여부. |
| devMode | Boolean | `TELEGRAM_BOT_TOKEN=dev-placeholder` 로그 출력 모드 여부. |
| statusCode | Integer | 실제 Telegram API HTTP 상태 코드. dev-placeholder/설정 실패 시 null 가능. |
| errorMessage | String | 실패 원인. 예: 토큰 누락, Chat ID 누락, Telegram API HTTP 오류. |
| logId | Long | 테스트 발송 결과가 저장된 `notification_log` ID. |
