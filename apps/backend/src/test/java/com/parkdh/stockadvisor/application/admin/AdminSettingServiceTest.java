...
class AdminSettingServiceTest { // 관리자 설정 서비스 테스트 클래스를 정의한다.
    ...
    void getSettingsReturnsAllSettingsSortedByKey() { // 설정 목록 정렬 테스트를 정의한다.
        ...
        ResultDto<?> result = adminSettingService.getSettings(); // 설정 목록을 조회한다.
        assertThat(result.code()).isEqualTo(200); // 성공 코드를 검증한다.
        assertThat(result.data().toString()).contains("key=a"); // 앞 키가 응답 데이터에 포함되는지 검증한다.
    } // 테스트 메서드를 종료한다.
