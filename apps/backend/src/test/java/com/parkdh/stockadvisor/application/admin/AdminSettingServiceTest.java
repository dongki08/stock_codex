...
import org.junit.jupiter.api.AfterEach; // 각 테스트 후 실행 어노테이션을 가져온다.
...
import org.springframework.security.authentication.TestingAuthenticationToken; // 테스트 인증 객체를 가져온다.
import org.springframework.security.core.context.SecurityContextHolder; // 보안 컨텍스트를 조작한다.
...
class AdminSettingServiceTest { // 관리자 설정 서비스 테스트 클래스를 정의한다.
    ...
    @AfterEach // 각 테스트 후 보안 컨텍스트를 정리한다.
    void tearDown() { // 테스트 격리 정리 메서드를 정의한다.
        SecurityContextHolder.clearContext(); // 남은 인증 정보를 제거한다.
    } // 테스트 격리 정리 메서드를 종료한다.
    ...
    @Test // 인증 주체가 요청 바디 actor보다 우선되는지 검증한다.
    void updateSettingUsesAuthenticatedActorForAuditLog() { // 인증 actor 우선 테스트를 정의한다.
        SecurityContextHolder.getContext().setAuthentication(new TestingAuthenticationToken("real-admin", "password", "ROLE_ADMIN")); // 인증 사용자명을 설정한다.
        AppSettingEntity entity = new AppSettingEntity("short.count", "{\"value\":3}", "단기 추천 개수", "admin"); // 기존 설정 엔티티를 생성한다.
        when(appSettingRepository.findById("short.count")).thenReturn(Optional.of(entity)); // 저장소가 기존 설정을 반환하게 한다.
        when(appSettingRepository.save(any(AppSettingEntity.class))).thenAnswer(invocation -> invocation.getArgument(0)); // 설정 저장소가 입력 엔티티를 반환하게 한다.
        when(auditLogRepository.save(any(AuditLogEntity.class))).thenAnswer(invocation -> invocation.getArgument(0)); // 감사 로그 저장소가 입력 엔티티를 반환하게 한다.
        adminSettingService.updateSetting("short.count", new AdminSettingUpdateRequest("{\"value\":4}", "spoofed-user")); // 설정을 수정한다.
        ArgumentCaptor<AuditLogEntity> captor = ArgumentCaptor.forClass(AuditLogEntity.class); // 감사 로그 인자 캡처를 준비한다.
        verify(auditLogRepository).save(captor.capture()); // 감사 로그 저장 호출을 검증하고 인자를 캡처한다.
        assertThat(captor.getValue().getActor()).isEqualTo("real-admin"); // 인증 사용자명이 감사 로그에 기록됐는지 검증한다.
        assertThat(entity.getUpdatedBy()).isEqualTo("real-admin"); // 설정 수정자도 인증 사용자명인지 검증한다.
    } // 테스트 메서드를 종료한다.
} // 테스트 클래스를 종료한다.
