...
public class MarketDataSyncController { // 시장 데이터 동기화 컨트롤러를 정의한다.
    ...
    @Operation(summary = "뉴스 조회", description = "멀티소스에서 저장된 KST 기준 당일 뉴스 제목/링크를 조회한다.") // Swagger 문서를 정의한다.
    ...
    @Operation(summary = "뉴스 멀티소스 동기화", description = "KR Google/Naver, US Yahoo/Google에서 KST 기준 당일 뉴스를 수집해 저장한다.") // Swagger 문서를 정의한다.
