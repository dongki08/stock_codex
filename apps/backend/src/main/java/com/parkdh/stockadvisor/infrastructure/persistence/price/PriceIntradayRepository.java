...
public interface PriceIntradayRepository extends JpaRepository<PriceIntradayEntity, String> { // 장중 가격 저장소 인터페이스를 정의한다.
    ...
    List<PriceIntradayEntity> findByMarketAndTickerAndTickAtBetweenOrderByTickAtAsc(String market, String ticker, LocalDateTime from, LocalDateTime to); // 보유 기간 중 장중 가격을 시간순으로 조회한다.
