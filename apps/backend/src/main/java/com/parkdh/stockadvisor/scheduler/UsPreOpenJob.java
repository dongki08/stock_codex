...
public class UsPreOpenJob { // 미국 장 전 브리핑 스케줄 작업을 정의한다.
    ...
    @Scheduled(cron = "0 0 22 * * MON-FRI", zone = "Asia/Seoul") // 매일 22:00 KST에 실행한다.
    public void run() { // 스케줄 작업을 실행한다.
        ...
        try { // 예외를 처리한다.
            ...
            DevRecommendationGenerateResponse recommendations = devRecommendationGenerateService.generate("NASDAQ", 3, 3); // NASDAQ 추천을 생성한다.
            String message = buildMessage(universe, quotes, daily, recommendations); // 브리핑 메시지를 구성한다.
            ...
            telegramClient.sendMessage("❌ UsPreOpenJob 오류: " + exception.getMessage()); // 오류 알림 메시지를 전송한다.
        } // 예외 처리를 종료한다.
    } // 스케줄 작업을 종료한다.

    private String buildMessage(MarketUniverseSyncResponse universe, MarketUniverseSyncResponse quotes, PriceDailySyncResponse daily, DevRecommendationGenerateResponse recommendations) { // US 프리오픈 메시지를 만든다.
        return "US PreOpen\n" // 제목을 추가한다.
                + "후보군 저장: " + universe.upsertedCount() + "개\n" // 후보군 저장 수를 추가한다.
                + "최근 가격 갱신: " + quotes.upsertedCount() + "개\n" // 최근 가격 갱신 수를 추가한다.
