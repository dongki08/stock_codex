...
import com.parkdh.stockadvisor.application.recommendation.ExitConfirmService; // Exit Confirm 서비스를 가져온다.
...
public class RecommendationController { // 추천 컨트롤러를 정의한다.
    ...
    private final ExitConfirmService exitConfirmService; // Exit Confirm 서비스 의존성을 보관한다.

    public RecommendationController(RecommendationService recommendationService, ExitConfirmService exitConfirmService) { // 생성자 주입을 정의한다.
        this.recommendationService = recommendationService; // 추천 서비스를 저장한다.
        this.exitConfirmService = exitConfirmService; // Exit Confirm 서비스를 저장한다.
    } // 생성자를 종료한다.
    ...
    @Operation(summary = "추천 Exit Confirm", description = """
            OPEN 추천의 현재가가 손절 위험 구간에 있는지 확인하고, 위험 구간이면 Codex 또는 규칙 기반 fallback으로 HOLD/CUT/TIGHTEN 판단을 반환한다.
            **사용 목적:**
            - 손절가 근접 상황에서 즉시 종료할지, 보유할지, 손절선을 조정할지 보조 판단
            **요청 파라미터:**
            - **id** *(Long, 필수)* : 추천 ID
            **반환 필드 (성공):**
            - **code** *(Integer)* : 200
            - **data** *(Object)* : currentPrice, stopPrice, distancePct, action, rationale
            **특징:**
            - price_intraday 최신값을 우선 사용하고, 없으면 price_daily 최신 종가를 사용한다.
            - `exit.riskBand.percent` 설정값으로 위험 구간 폭을 결정한다.
            """) // Swagger 문서를 정의한다.
    @PostMapping("/{id}/exit-confirm") // 추천 Exit Confirm 경로를 매핑한다.
    public ResultDto<?> confirmExit(@PathVariable Long id) { // 추천 Exit Confirm API를 정의한다.
        return ResultDto.success(exitConfirmService.confirm(id)); // 서비스 판단 결과를 성공 응답으로 래핑해 반환한다.
    } // 추천 Exit Confirm API를 종료한다.
} // 추천 컨트롤러를 종료한다.
