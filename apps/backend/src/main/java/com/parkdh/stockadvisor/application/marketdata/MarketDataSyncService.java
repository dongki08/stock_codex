...
import com.parkdh.stockadvisor.infrastructure.marketdata.us.YahooFinanceClient; // Yahoo Finance 일봉 클라이언트를 가져온다.
...
import java.time.DayOfWeek; // 요일 타입을 가져온다.
...
public class MarketDataSyncService { // 시장 데이터 동기화 서비스를 정의한다.
    ...
    private final YahooFinanceClient yahooFinanceClient; // Yahoo Finance 일봉 클라이언트 의존성을 보관한다.
    private final AppSettingRepository appSettingRepository; // 설정 저장소 의존성을 보관한다.
    private final ObjectMapper objectMapper; // JSON 파싱 도구 의존성을 보관한다.
    ...
    public PriceDailySyncResponse syncDailyPrices(String market, Integer limit, Integer days) { // 후보군의 일봉 가격을 동기화한다.
        return syncDailyPricesInternal(market, limit, days, true, Integer.MAX_VALUE); // 일반 수동/백필 모드는 히스토리 부트스트랩을 허용한다.
    } // 일봉 가격 동기화를 종료한다.

    @Transactional
    public PriceDailySyncResponse syncDailyPricesForPreOpen(String market, Integer limit, Integer days) {
        return syncDailyPricesInternal(market, limit, days, false, 5); // 장전 추천 직전에는 대량 bootstrap/catch-up을 금지한다.
    }

    @Transactional
    public PriceDailySyncResponse syncKrxDailyPricesForDate(String market, LocalDate tradeDate) {
        String safeMarket = market == null || market.isBlank() ? "KOSPI" : market;
        List<KrxOpenApiClient.KrxDailyTradeRow> fetchedRows = krxOpenApiClient.fetchDailyTrades(safeMarket, tradeDate);
        java.util.ArrayList<PriceDailyEntity> savedRows = new java.util.ArrayList<>();
        for (KrxOpenApiClient.KrxDailyTradeRow row : fetchedRows) {
            savedRows.add(upsertDailyPrice(new DailyPriceRow(row.ticker(), row.market(), row.tradeDate(), row.openPrice(), row.highPrice(), row.lowPrice(), row.closePrice(), row.volume(), row.turnover(), "KRX_OPENAPI")));
            upsertUniverseFromKrxRow(row);
        }
        List<String> sampleKeys = savedRows.stream().map(PriceDailyEntity::getPriceKey).limit(20).toList();
        return new PriceDailySyncResponse(safeMarket, fetchedRows.size(), 1, 0, 0, fetchedRows.size(), savedRows.size(), tradeDate, "KRX_OPENAPI_DAILY", sampleKeys);
    }

    @Transactional
    public PriceDailySyncResponse syncKrxDailyPrices(String market, LocalDate from, LocalDate to) {
        String safeMarket = market == null || market.isBlank() ? "KOSPI" : market;
        ...
        int fetchedCount = 0;
        int upsertedCount = 0;
        java.util.ArrayList<String> sampleKeys = new java.util.ArrayList<>();
        while (!cursor.isAfter(to)) {
            if (cursor.getDayOfWeek() != DayOfWeek.SATURDAY && cursor.getDayOfWeek() != DayOfWeek.SUNDAY) {
                PriceDailySyncResponse response = syncKrxDailyPricesForDate(safeMarket, cursor);
                requestedCount += response.requestedTickerCount();
                fetchedCount += response.fetchedCount();
                upsertedCount += response.upsertedCount();
                candidateCount = Math.max(candidateCount, response.candidateCount());
                response.samplePriceKeys().stream().limit(Math.max(0, 20 - sampleKeys.size())).forEach(sampleKeys::add);
            }
            cursor = cursor.plusDays(1);
        }
        return new PriceDailySyncResponse(safeMarket, candidateCount, requestedCount, 0, 0, fetchedCount, upsertedCount, to, "KRX_OPENAPI_RANGE", sampleKeys);
    }

    private PriceDailySyncResponse syncDailyPricesInternal(String market, Integer limit, Integer days, boolean bootstrapMissing, int maxCatchupDays) {
        ...
        LocalDate targetDate = previousBusinessDay(LocalDate.now()); // 장전 추천 기준으로 안전한 목표 거래일을 정한다.
        ...
        java.util.ArrayList<PriceDailyEntity> rows = new java.util.ArrayList<>(); // 저장된 일봉 목록을 보관한다.
        int requestedTickerCount = 0; // 외부 조회 종목 수를 보관한다.
        int skippedUpToDateCount = 0; // 최신 데이터 보유 종목 수를 보관한다.
        int skippedNoHistoryCount = 0; // 장전 bootstrap 금지로 건너뛴 종목 수를 보관한다.
        int fetchedCount = 0; // 외부에서 받은 일봉 행 수를 보관한다.
        for (MarketUniverseEntity candidate : candidates) { // 후보별로 증분 필요 여부를 계산한다.
            SyncWindow window = resolveSyncWindow(candidate, targetDate, safeDays, bootstrapMissing, maxCatchupDays); // 조회 창을 결정한다.
            if (window.status() == SyncStatus.UP_TO_DATE) { // 이미 최신이면 건너뛴다.
                skippedUpToDateCount++;
                continue;
            }
            if (window.status() == SyncStatus.SKIP_NO_HISTORY) { // 장전 모드에서 히스토리가 없거나 너무 오래됐으면 건너뛴다.
                skippedNoHistoryCount++;
                continue;
            }
            requestedTickerCount++;
            List<DailyPriceRow> fetchedRows = fetchDailyPrices(candidate, window.from(), targetDate, safeDays); // 필요한 구간만 외부 조회한다.
            fetchedCount += fetchedRows.size();
            for (DailyPriceRow row : fetchedRows) {
                rows.add(upsertDailyPrice(row)); // 일봉을 저장 또는 갱신한다.
            }
        }
        ...
        String mode = bootstrapMissing ? "BOOTSTRAP_ALLOWED" : "INCREMENTAL_ONLY";
        return new PriceDailySyncResponse(safeMarket, candidates.size(), requestedTickerCount, skippedUpToDateCount, skippedNoHistoryCount, fetchedCount, rows.size(), targetDate, mode, sampleKeys); // 동기화 결과를 반환한다.
    } // 내부 일봉 가격 동기화를 종료한다.

    private SyncWindow resolveSyncWindow(MarketUniverseEntity candidate, LocalDate targetDate, int fullDays, boolean bootstrapMissing, int maxCatchupDays) { // 티커별 조회 시작일을 결정한다.
        List<PriceDailyEntity> latest = priceDailyRepository.findByMarketAndTickerOrderByTradeDateDesc(
                candidate.getMarket(), candidate.getTicker(), PageRequest.of(0, 1)); // 최신 일봉 1건을 조회한다.
        if (latest.isEmpty()) { // 데이터가 없으면 전체 기간을 조회한다.
            if (!bootstrapMissing) {
                return new SyncWindow(SyncStatus.SKIP_NO_HISTORY, null);
            }
            return new SyncWindow(SyncStatus.REQUEST, targetDate.minusDays(fullDays));
        }
        LocalDate latestDate = latest.get(0).getTradeDate(); // 최신 거래일을 가져온다.
        if (!latestDate.isBefore(targetDate)) { // 목표 거래일까지 이미 있으면 스킵한다.
            return new SyncWindow(SyncStatus.UP_TO_DATE, null);
        }
        long catchupDays = java.time.temporal.ChronoUnit.DAYS.between(latestDate, targetDate); // 따라잡아야 할 날짜 차이를 계산한다.
        if (!bootstrapMissing && catchupDays > maxCatchupDays) { // 장전 모드에서 큰 갭은 백필 잡으로 넘긴다.
            return new SyncWindow(SyncStatus.SKIP_NO_HISTORY, null);
        }
        return new SyncWindow(SyncStatus.REQUEST, latestDate.plusDays(1)); // 최신일 다음날부터 조회한다.
    } // 티커별 조회 시작일 결정을 종료한다.
    ...
    private List<DailyPriceRow> fetchDailyPrices(MarketUniverseEntity candidate, LocalDate from, LocalDate to, int maxRows) { // 후보군별 외부 일봉을 조회한다.
        ...
        if ("KOSPI".equals(market) || "KOSDAQ".equals(market)) { // 한국 시장인지 확인한다.
            try { Thread.sleep(getKisDelayMs()); } catch (InterruptedException e) { Thread.currentThread().interrupt(); } // KIS 초당 거래 한도 준수 딜레이 — setting: collection.kis.dailyPrice.delayMs.
        ...
        if ("NASDAQ".equals(market) || "NYSE".equals(market)) { // 미국 시장인지 확인한다.
            return yahooFinanceClient.fetchDailyPrices(candidate.getTicker(), from, to, maxRows).stream()
                    .map(price -> new DailyPriceRow(candidate.getTicker(), market, price.tradeDate(), price.openPrice(), price.highPrice(), price.lowPrice(), price.closePrice(), price.volume(), price.turnover(), "YAHOO")) // Yahoo Finance 일봉을 공통 행으로 변환한다.
                    .toList(); // 목록으로 수집한다.
        } // 미국 시장 확인을 종료한다.
    ...
    private void upsertUniverseFromKrxRow(KrxOpenApiClient.KrxDailyTradeRow row) {
        String key = MarketUniverseEntity.buildKey(row.market(), row.ticker());
        marketUniverseRepository.findById(key)
                .map(existing -> {
                    existing.update(row.name(), existing.getSector(), row.marketCap(), row.turnover(), row.closePrice(), true, "KRX_OPENAPI", row.tradeDate());
                    return marketUniverseRepository.save(existing);
                })
                .orElseGet(() -> marketUniverseRepository.save(new MarketUniverseEntity(row.ticker(), row.market(), row.name(), null, row.marketCap(), row.turnover(), row.closePrice(), true, "KRX_OPENAPI", row.tradeDate())));
    }
    ...
    private int getKisDelayMs() {
        return appSettingRepository.findById("collection.kis.dailyPrice.delayMs")
                .map(e -> {
                    try { return objectMapper.readTree(e.getValueJson()).path("value").asInt(600); }
                    catch (Exception ex) { return 600; }
                })
                .orElse(600);
    }
    ...
    private record SyncWindow(SyncStatus status, LocalDate from) {
    }
