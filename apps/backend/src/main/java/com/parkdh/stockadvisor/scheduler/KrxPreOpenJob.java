...
public class KrxPreOpenJob { // KRX 장 전 브리핑 스케줄 작업을 정의한다.
    ...
    private static final int MAX_TRADING_DAY_LOOKBACK = 10;
    ...
    public void trigger() {
        ...
        try { // 예외를 처리한다.
            ...
            PriceDailySyncResponse kospiDaily = syncKrxDailyForPreOpen(today, "KOSPI"); // KRX OpenAPI 날짜 단위 수집을 우선 사용한다.
            PriceDailySyncResponse kosdaqDaily = syncKrxDailyForPreOpen(today, "KOSDAQ"); // KOSDAQ도 동일하게 처리한다.
            int dailyUpsertedCount = kospiDaily.upsertedCount() + kosdaqDaily.upsertedCount();
            ...
            List<String> recLines = recommendations.recommendations().stream()
                    .map(r -> {
                        ...
                        String relation = contextRelationAnalysisService.find(r.market(), r.ticker(), today)
                                .map(a -> "\nAI 관계 [" + a.getDirection() + "/" + a.getRiskLevel() + " " + a.getRelationScore() + "] " + a.getSummary())
                                .orElse("");
                        String exitAt = r.expectedExitAt() != null ? "\n📅 기간 ~" + r.expectedExitAt() : "";
            ...
            notificationService.sendSchedulerEvent(
                    ...
                    List.of(
                            ...
                            new NotificationMetric("일봉 저장", dailyUpsertedCount + "개"),
                            new NotificationMetric("추천 생성", recommendations.generatedRecommendationCount() + "건")
                    ),
                    recLines
            );
            log.info("KrxPreOpenJob 완료. universeSaved={}, dailySaved={}, recommendations={}", universe.upsertedCount(), dailyUpsertedCount, recommendations.generatedRecommendationCount()); // 작업 완료 로그를 출력한다.
            ...
            notificationService.sendSchedulerError("krx-preopen", LocalDate.now(SEOUL_ZONE), exception.getMessage());
        } // 예외 처리를 종료한다.
    } // 스케줄 작업을 종료한다.

    PriceDailySyncResponse syncKrxDailyForPreOpen(LocalDate today, String market) {
        LocalDate targetDate = previousBusinessDay(today);
        try {
            for (int attempt = 0; attempt < MAX_TRADING_DAY_LOOKBACK; attempt++) {
                PriceDailySyncResponse response = marketDataSyncService.syncKrxDailyPricesForDate(market, targetDate);
                if (response.fetchedCount() != null && response.fetchedCount() > 0) {
                    return response;
                }
                log.info("KrxPreOpenJob {} KRX OpenAPI 데이터 없음. 이전 거래일 탐색. targetDate={}", market, targetDate);
                targetDate = previousBusinessDay(targetDate);
            }
            log.warn("KrxPreOpenJob {} KRX OpenAPI 최근 거래일 데이터를 찾지 못함. KIS 증분 fallback 실행.", market);
            return marketDataSyncService.syncDailyPricesForPreOpen(market, 120, 100);
        } catch (Exception exception) {
            log.warn("KrxPreOpenJob {} KRX OpenAPI 일봉 동기화 실패. KIS 증분 fallback 실행. error={}", market, exception.getMessage());
            return marketDataSyncService.syncDailyPricesForPreOpen(market, 120, 100);
        }
    }
