...
public class UniverseFeatureBuilder { // 후보군 feature 빌더를 정의한다.
    ...
    private ContextSnapshot buildContextSnapshotAsOf(MarketUniverseEntity entity, ContextWeights weights, LocalDate asOf) { // asOf 이하 컨텍스트 feature를 계산한다.
        LocalDateTime asOfEnd = asOf.atTime(23, 59, 59); // asOf 날짜 말까지를 기준으로 한다.
        List<NewsArticleEntity> news = newsArticleRepository.findByMarketAndTickerAndPublishedAtLessThanEqualOrderByPublishedAtDesc(entity.getMarket(), entity.getTicker(), asOfEnd, PageRequest.of(0, 10)); // asOf 이하 뉴스를 조회한다.
        ...
        return new ContextSnapshot(newsScore, disclosureScore, macroScore, fundamentalScore, contextScore, news.size(), disclosures.size(), macroObservations.size(), fundamentals.size()); // PIT 컨텍스트 스냅샷을 반환한다.
