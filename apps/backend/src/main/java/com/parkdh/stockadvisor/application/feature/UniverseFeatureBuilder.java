...
public class UniverseFeatureBuilder { // 후보군 feature 빌더를 정의한다.
    ...
    private List<MarketUniverseEntity> findUniverse(String market) { // 시장 조건으로 후보군을 조회한다.
        ...
        return marketUniverseRepository.findByMarketAndTradable(market, true); // 특정 시장 거래 가능 후보군을 반환한다.
    } // 후보군 조회를 종료한다.
    ...
    private UniverseFeature buildFeature(MarketUniverseEntity entity) { // 단일 후보군 feature를 생성한다.
        ...
        TechnicalSnapshot technical = buildTechnicalSnapshot(priceHistory); // 일봉 기반 기술적 feature를 계산한다.
        ...
        ContextSnapshot context = buildContextSnapshot(entity); // 뉴스/공시/매크로 컨텍스트 feature를 계산한다.
        int totalScore = Math.min(100, Math.round(liquidityScore * 0.20f + priceScore * 0.10f + technical.technicalScore() * 0.30f + context.contextScore() * 0.15f + context.fundamentalScore() * 0.10f + dataQualityScore * 0.15f)); // 가중 종합 점수를 계산한다.
    ...
    private ContextSnapshot buildContextSnapshot(MarketUniverseEntity entity) { // 뉴스/공시/매크로 컨텍스트 feature를 계산한다.
        ...
        int contextScore = Math.min(100, Math.round(newsScore * 0.40f + disclosureScore * 0.18f + macroScore * 0.25f + fundamentalScore * 0.17f)); // 컨텍스트 종합 점수를 계산한다.
    ...
    private TechnicalSnapshot buildTechnicalSnapshot(List<PriceDailyEntity> priceHistory) { // 일봉 히스토리로 기술적 feature를 계산한다.
        ...
        int technicalScore = Math.min(100, Math.round(movingAverageScore * 0.40f + rsiScore * 0.35f + volumeScore * 0.25f)); // 기술적 종합 점수를 계산한다.
