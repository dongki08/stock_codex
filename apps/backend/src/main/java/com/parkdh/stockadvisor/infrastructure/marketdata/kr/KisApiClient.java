...
public class KisApiClient { // 한국투자증권 API 클라이언트를 정의한다.
    ...
    public Optional<KisCurrentPrice> fetchCurrentPrice(String ticker) { // 국내 주식 현재가를 조회한다.
        ...
        return fetchCurrentPriceWithRetry(ticker, 3); // 재시도 로직을 포함해 현재가를 조회한다.
    } // 현재가 조회를 종료한다.

    private Optional<KisCurrentPrice> fetchCurrentPriceWithRetry(String ticker, int remaining) { // EGW00201 재시도 포함 현재가 조회를 수행한다.
        ...
        try { // 네트워크 예외를 처리한다.
            ...
            if (response.statusCode() < 200 || response.statusCode() >= 300) { // 성공 상태 코드인지 확인한다.
                ...
                log.warn("KIS 현재가 조회 실패. ticker={}, status={}, body={}", ticker, response.statusCode(), body.substring(0, Math.min(300, body.length())));
                if (remaining > 0 && body.contains("EGW00201")) { // 초당 거래 한도 초과 시 재시도한다.
                    ...
                    Thread.sleep(1000);
                    return fetchCurrentPriceWithRetry(ticker, remaining - 1);
                }
                throw new CustomException("KIS 현재가 조회에 실패했습니다. ticker=" + ticker + ", status=" + response.statusCode() + ", body=" + body.substring(0, Math.min(200, body.length())), 502); // 외부 API 실패 예외를 던진다.
            } // 상태 코드 확인을 종료한다.
