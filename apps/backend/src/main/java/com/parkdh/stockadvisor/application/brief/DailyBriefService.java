...
public class DailyBriefService { // 데일리 브리프 서비스를 정의한다.
    ...
    private static final int DEFAULT_PROMPT_MAX_CHARS = 6000; // Codex 프롬프트 기본 최대 길이를 정의한다.
    private static final String TRUNCATED_MARKER = "\n\nTRUNCATED_TO_MAX_CHARS"; // 프롬프트 절단 표시를 정의한다.
    ...
    private BriefContext buildContext(String marketTrack) { // 브리프 입력 컨텍스트를 구성한다.
        ...
        List<NewsArticleEntity> news = markets.stream()
                .flatMap(market -> newsArticleRepository.findByMarketAndPublishedAtBetweenOrderByPublishedAtDesc(market, today.atStartOfDay(), today.atTime(LocalTime.MAX), PageRequest.of(0, 10)).stream()) // 시장별 당일 뉴스를 조회한다.
    ...
    private String buildPrompt(BriefContext context, String additionalPrompt) { // Codex 입력 프롬프트를 구성한다.
        ...
        builder.append("OPERATING_BRIEF_CONTRACT\n");
        builder.append("- 목적: 아래 실제 수집 데이터만 사용해 ").append(context.marketTrack()).append(" 데일리 브리프를 작성한다.\n");
        builder.append("- 출력: 마크다운, 700자 이내, 섹션은 시장 요약/추천 점검/리스크/오늘 할 일.\n");
        builder.append("- 금지: 제공된 데이터에 없는 사실, 가격, 뉴스, 공시, 재무 지표를 추정하지 않는다.\n");
        builder.append("- 누락 데이터: 필요한 근거가 없으면 반드시 '데이터 없음'이라고 쓴다.\n");
        builder.append("- 운영 판단: 실행 가능한 점검 항목과 리스크를 우선한다.\n\n");
        ...
        return capPrompt(builder.toString(), resolvePromptMaxChars()); // 프롬프트를 길이 제한 후 반환한다.
    } // Codex 입력 프롬프트 구성을 종료한다.

    private String capPrompt(String prompt, int maxChars) { // Codex 입력 프롬프트 길이를 제한한다.
        int safeMax = Math.max(800, maxChars); // 운영 계약이 잘리지 않도록 최소 길이를 보장한다.
        if (prompt.length() <= safeMax) { // 제한 이하인지 확인한다.
            return prompt; // 원문을 반환한다.
        } // 제한 확인을 종료한다.
        int contentLimit = Math.max(0, safeMax - TRUNCATED_MARKER.length()); // 절단 표시 공간을 제외한 본문 길이를 계산한다.
        return prompt.substring(0, contentLimit) + TRUNCATED_MARKER; // 절단 표시를 붙여 반환한다.
    } // Codex 입력 프롬프트 길이 제한을 종료한다.
    ...
    private int resolvePromptMaxChars() { // 데일리 브리프 프롬프트 최대 길이 설정을 조회한다.
        return appSettingRepository.findById("dailybrief.prompt.maxChars")
                .map(AppSettingEntity::getValueJson)
                .map(valueJson -> extractIntValue(valueJson, DEFAULT_PROMPT_MAX_CHARS))
                .orElse(DEFAULT_PROMPT_MAX_CHARS); // 설정이 없으면 기본 최대 길이를 반환한다.
    } // 데일리 브리프 프롬프트 최대 길이 설정 조회를 종료한다.
    ...
    private int extractIntValue(String valueJson, int defaultValue) { // 설정 JSON에서 value 정수를 추출한다.
        try { // JSON 파싱 예외를 처리한다.
            JsonNode node = objectMapper.readTree(valueJson); // 설정 JSON을 파싱한다.
            int value = node.path("value").asInt(defaultValue); // value 필드를 읽는다.
            return value > 0 ? value : defaultValue; // 양수이면 사용하고 아니면 기본값을 반환한다.
        } catch (Exception exception) { // 파싱 예외를 잡는다.
            return defaultValue; // 파싱 실패 시 기본값을 반환한다.
        } // 예외 처리를 종료한다.
    } // 설정 JSON value 정수 추출을 종료한다.
